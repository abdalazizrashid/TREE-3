...
((nil . ((sentence-end-double-space . t)
         ...
         (compile-command . "darwin-rebuild test --flake .#m1")))
 (nix-mode
  . ((mode . nixpkgs-fmt-on-save)
     (eglot-workspace-configuration
      . ((options
          (nixos
           :expr (format "(builtins.getFlake \"%s\").darwinConfigurations.m1.options"
                         (expand-file-name "." default-directory)))
          (Home-manager
           :expr (format "(builtins.getFlake \"%s\").darwinConfigurations.m1.options.home-manager"
                         (expand-file-name "." default-directory)))))))))
