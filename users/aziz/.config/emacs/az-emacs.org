...
#+begin_src emacs-lisp :tangle "init.el"
  ...
  (defvar publish-dir "~/Sources/tree-3/users/aziz/www/")
  ...
  (setq org-publish-project-alist
  	 ...
  	 :auto-sitemap nil
  	 :exclude ".*.org"
  	 :include ("pkg.org" "az-emacs.org", "dft.org" "infra/infra.org" "SIP.org")
  	 :html-postamble nil
  	 :section-numbers nil
...
#+begin_src elisp
  ...
  (defvar identifier-type "DOI" "Type of identifier: DOI or ISBN.")
  (defvar output-method "clipboard" "Output method: clipboard or file.")
  (defvar identifier "" "The DOI or ISBN to retrieve.")

  (defun retrieve-crossref-metadata-core (identifier type output-method)
    "Retrieve metadata from CrossRef using IDENTIFIER of TYPE and process OUTPUT-METHOD."
    (let* ((url (if (string-equal type "DOI")
                    (format "https://api.crossref.org/works/%s" identifier)
                  (format "https://api.crossref.org/works?filter=isbn:%s" identifier))))
      (with-current-buffer (url-retrieve-synchronously url)
        (goto-char url-http-end-of-headers)
        (let* ((json-object (json-read))
               (message (cdr (assoc 'message json-object)))
               (items (if (string-equal type "ISBN")
                          (cdr (assoc 'items message))
                        nil))
               (work (if items
                         (elt items 0)
                       message))
               (title-list (cdr (assoc 'title work)))
          ...
          (cond
           ((string-equal output-method "clipboard")
            (kill-new rec-entry)
            (message "Rec record copied to clipboard"))
           ((string-equal output-method "file")
  	  ;; Append to the rec to clipboard
  	  (kill-new rec-entry)
  	  (message "Rec record copied to clipboard"))
           (t
            (message "Invalid output method")))))))


  (defun retrieve-crossref-metadata-execute ()
    "Execute the metadata retrieval with selected options."
    (interactive)
    (retrieve-crossref-metadata-core identifier identifier-type output-method))

  (transient-define-prefix retrieve-crossref-metadata-transient ()
    "Retrieve metadata from CrossRef and process it."
    ["Retrieve Metadata"
     ("t" "Identifier Type" 
      (lambda () (interactive)
        (transient-argument-set 'identifier-type
                                (completing-read "Identifier Type: " '("DOI" "ISBN") nil t))))
     ("o" "Output Method"
      (lambda () (interactive)
        (transient-argument-set 'output-method
                                (completing-read "Output Method: " '("clipboard" "file") nil t))))
     ("i" "Identifier"
      (lambda () (interactive)
        (transient-argument-set 'identifier
                                (read-string "Enter Identifier: "))))
     ("g" "Go" retrieve-crossref-metadata-execute)]
    (interactive)
    (setq identifier-type "DOI")
    (setq output-method "clipboard")
    (setq identifier "")
    (transient-setup 'retrieve-crossref-metadata-transient))
#+end_src
#+begin_src elisp :tangle init.el :tangle init.el
